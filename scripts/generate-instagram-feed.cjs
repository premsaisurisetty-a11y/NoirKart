...
const tempOutFile = path.resolve(__dirname, 'temp-products-ig.cjs');
const productsFile = path.resolve(__dirname, '../src/app/data/products.ts');
const targetFile = path.resolve(__dirname, '../public/instagram-feed.xml');

console.log('Compiling products data using esbuild for Instagram feed...');
try {
  ...
  xml += `  <channel>\n`;
  xml += `    <title>NoirKart Instagram Product Catalog</title>\n`;
  ...
  console.log(`Instagram Catalog Feed generated successfully at: ${targetFile}`);
