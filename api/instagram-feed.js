...
export default async function handler(req, res) {
  try {
    ...
    if (products.length === 0) {
      ...
      const staticXmlPath = path.join(process.cwd(), 'public/instagram-feed.xml');
    ...
    console.log(`Successfully fetched ${products.length} live products from Firestore. Generating Instagram feed...`);
    
    let xml = `<?xml version="1.0" encoding="utf-8"?>\n`;
    xml += `<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">\n`;
    xml += `  <channel>\n`;
    xml += `    <title>NoirKart Live Instagram Product Catalog</title>\n`;
